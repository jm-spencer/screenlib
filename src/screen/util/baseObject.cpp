#include "screen/util/baseObject.hpp"

namespace screen {

BaseObject::BaseObject(lv_obj_t *parent, lv_style_t *style)
    : xScalar(1), yScalar(1)
{
  obj = lv_cont_create(parent, NULL);
  lv_obj_set_style(obj, style);
}

BaseObject::~BaseObject()
{
  lv_obj_del(obj);
}

void BaseObject::setX(int x)
{
  lv_obj_set_x(obj, x);
}

void BaseObject::setY(int y)
{
  lv_obj_set_y(obj, y);
}

void BaseObject::setPos(int x, int y)
{
  lv_obj_set_pos(obj, x, y);
}

void BaseObject::hide(bool hide)
{
  lv_obj_set_hidden(obj, hide);
}

void BaseObject::setWidth(uint32_t iwidth)
{
  lv_obj_set_width(obj, iwidth);
  xScalar = iwidth / 100.0;
  align();
}

void BaseObject::setHeight(uint32_t iheight)
{
  lv_obj_set_height(obj, iheight);
  yScalar = iheight / 100.0;
  align();
}

void BaseObject::setSize(uint32_t iwidth, uint32_t iheight)
{
  lv_obj_set_size(obj, iwidth, iheight);
  xScalar = iwidth / 100.0;
  yScalar = iheight / 100.0;
  align();
}

}  // namespace screen
